...
class main:
	...
	def mainthread(self):
		...
		plt.xlim((-20,20))
		plt.ylim((-10,10))
		for k in range(1000):						# Run algorithm for 1000 iterations
			...
			for i in range(self.numagents):
				...
				agentclass.getObstacles(self.obstacles)
				d = (agentclass.A).dot(np.transpose(np.ones(self.numagents)))
				D = np.diag(d)
				L = D - agentclass.A
				xfin = agentclass.PIaverageconsensus(i, L,agentclass.N, agentclass.ND, agentclass.x, agentclass.y1, agentclass.y2, agentclass.w1, agentclass.w2)
				uk = agentclass.controlLaw(agentclass.A[i], xfin, agentclass.N, agentclass.ND, self.xcoord[i], self.ycoord[i])
				if i==0:
					uk[0], uk[1] = 0.0,0.0
				if i != 0:							# Update the graph only for those points using the control input that are not being controlled. 
